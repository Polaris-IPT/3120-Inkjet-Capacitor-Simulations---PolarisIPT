function inkjet_gui_capacitor_zdeflect_fixed_timed_collisions
% Vertical-only GUI (I-shape) with plate-collision detection (Option 1).
% Zero-thickness square plates in xy-plane at z = +/- W/2.
% Collision places a red 'X' on the plate and the droplet is removed.
    ...
    params.L2_mm = 1.25;
    params.W_mm  = 1.0;          % plate separation (mm)
    params.vx = 10;              % m/s
    ...
    % ----------- GUI setup -----------
    hFig = figure('Name','Inkjet Vertical GUI (I) - Collisions','NumberTitle','off',...
    ...
    xlabel(hAxV,'time (s)'); ylabel(hAxV,'V (V)'); title(hAxV,'Voltage staircase V_z(t)'); grid(hAxV,'on');
    ...
    addLabel('Droplet charge q (C):', ctrlY); hQ = addEdit(params.q, ctrlY, @q_cb); ctrlY = ctrlY - step;
    addLabel('Droplet diameter (um):', ctrlY); hDiam = addEdit(params.droplet_d*1e6, ctrlY, @diam_cb); ctrlY = ctrlY - step;
    addLabel('Droplet speed vx (m/s):', ctrlY); hVx = addEdit(params.vx, ctrlY, @vx_cb); ctrlY = ctrlY - step;
    addLabel('Firing freq f (Hz):', ctrlY); hF = addEdit(params.f_fire, ctrlY, @f_cb); ctrlY = ctrlY - step;
    addLabel('Max voltage Vmax (V):', ctrlY); hVmax = addEdit(params.Vmax, ctrlY, @vmax_cb); ctrlY = ctrlY - step;
    addLabel('Plate separation W (mm):', ctrlY); hW = addEdit(params.W_mm, ctrlY, @W_cb); ctrlY = ctrlY - step;
    addLabel('Cap start x (mm):', ctrlY); hCapStart = addEdit(params.cap_start_mm, ctrlY, @capstart_cb); ctrlY = ctrlY - step;
    addLabel('L1 (mm):', ctrlY); hL1 = addEdit(params.L1_mm, ctrlY, @L1_cb); ctrlY = ctrlY - step;
    addLabel('L2 (mm):', ctrlY); hL2 = addEdit(params.L2_mm, ctrlY, @L2_cb); ctrlY = ctrlY - step;
    addLabel('Paper x (mm):', ctrlY); hD = addEdit(params.D_mm, ctrlY, @D_cb); ctrlY = ctrlY - step;
    addLabel('Paper width (y) mm:', ctrlY); hPy = addEdit(params.paper_w_mm, ctrlY, @py_cb); ctrlY = ctrlY - step;
    addLabel('Paper height (z) mm:', ctrlY); hPz = addEdit(params.paper_h_mm, ctrlY, @pz_cb); ctrlY = ctrlY - step;
    addLabel('DPI:', ctrlY); hDPI = addEdit(params.dpi, ctrlY, @dpi_cb); ctrlY = ctrlY - step;
    addLabel('Animation speed (mult):', ctrlY); hSpeed = addEdit(params.animation_speed, ctrlY, @speed_cb); ctrlY = ctrlY - step;

    % Buttons and status/timing text
    ...
    function drawScene()
        ...
        title(hAx3,'3D View: Nozzle -> Z-plates -> Paper');
        drawnow;
    end

    % ---------------- compute voltages and staircase ----------------
    function computeV_cb(~,~)
        ...
        V_clip = V_required; V_clip(abs(V_clip) > params.Vmax) = sign(V_clip(abs(V_clip)>params.Vmax)) .* params.Vmax;
    ...
    function start_cb(~,~)
        ...
        % start total I timer
        timing_metrics('start_i');
        run_I_sim(simData);
        tI = timing_metrics('stop_i');
    ...
    % ---------------- core simulation for I (with collisions) ----------------
    function run_I_sim(simData)
        ...
        z_top_m = (params.W_mm/2) * mm2m;
        z_bot_m = -(params.W_mm/2) * mm2m;

        first_drop_recorded = false; % indicates whether first successful impact occurred

        for k = 1:simData.N_dots
            if stopRequested, set(hStatus,'String','Stopped'); break; end

            % start-first-drop timer at first fired droplet
            if k == 1
            ...
            for s = 1:steps
                % update acceleration state (only affects velocity, not collision checks)
                ...
                % collision check with top plate
                if (x >= x_cap_start_m && x <= x_cap_end_m) && (y >= y_plate_min_m && y <= y_plate_max_m)
                    % top
                    if z + r_drop >= z_top_m
                        % collision with top plate
                        collided = true;
                        coll_x_mm = x / mm2m;
                        coll_y_mm = y / mm2m;
                        coll_z_mm = z_top_m / mm2m;
            ...
            % place persistent impact on paper
            landing_z_mm = z / mm2m;
            landing_y_mm = y / mm2m;
            ...
            % stop-first-drop timer at first successful paper impact
